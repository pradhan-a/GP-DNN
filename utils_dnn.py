...
class GP_DNN_marginal_neglikelihood_loss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, x_tilde_pred):
        y_true_cokrig = tf.concat(
            [
                tf.expand_dims(
                    tf.gather(
                        y_true[:, 0], tf.squeeze(tf.where(y_true[:, -1] == i)), axis=0
                    ),
                    axis=-1,
                )
        ...
        x_tilde_pred_cokrig = [
            tf.gather(x_tilde_pred, tf.squeeze(tf.where(y_true[:, -1] == i)), axis=0)
            for i in range(self.num_coef)
        ]
        ...
        x_tilde_pred_cov = cokriging_covariance(
            ...
            self.noise_var,
            self.corr_coef,
            is_training_cov=True,
        )
        x_tilde_pred_cov = tf.concat(
            [
                tf.concat(x_tilde_pred_cov[i], axis=-1)
                for i in range(len(x_tilde_pred_cov))
            ],
            axis=0,
        )

        data_fit = 0.5 * tf.linalg.matmul(
            tf.linalg.matmul(
                y_true_cokrig, tf.linalg.inv(x_tilde_pred_cov), transpose_a=True
            ),
            y_true_cokrig,
        )
...
class GP_DNN_likelihood(tf.keras.metrics.Metric):
    def __init__(
        ...
        y_train_obs,
        noise_var=[0.5, 0.1],
        corr_coef=np.array([[1, 0.3], [0.3, 1]]),
        eval_var=[0],
        name="GP_DNN_likelihood",
        **kwargs
    ):
    ...
    def update_state(self, y_true, x_tilde_pred, sample_weight=None):
        ...
        x_tilde_train_cokrig = [
            tf.gather(
                x_tilde_train,
                tf.squeeze(tf.where(self.y_train_obs[:, -1] == i)),
                axis=0,
            )
            for i in range(self.num_coef)
        ]
        ...
        x_tilde_train_cov = tf.concat(
            [
                tf.concat(x_tilde_train_cov[i], axis=-1)
                for i in range(len(x_tilde_train_cov))
            ],
            axis=0,
        )
        ...
        ####
        x_tilde_pred_cov = tf.concat(
            [tf.concat(x_tilde_pred_cov[i], axis=-1) for i in range(self.num_coef)],
            axis=0,
        )
        x_tilde_pred_pred_cov = tf.concat(
            [
                tf.concat(x_tilde_pred_pred_cov[i], axis=-1)
                for i in range(self.num_coef)
            ],
            axis=0,
        )
        ...
        y_pred = tf.linalg.matmul(
            tf.linalg.matmul(x_tilde_pred_cov, x_tilde_train_cov_inv), y_train
        )
...
class GP_DNN_RMSE(tf.keras.metrics.Metric):
    ...
    def update_state(self, y_true, x_tilde_pred, sample_weight=None):
        ...
        for i in self.eval_var:
            ...
            krig_weights = tf.linalg.solve(
                x_tilde_train_cov, tf.transpose(x_tilde_pred_cov1)
            )
...
def cokriging_covariance(
    x_tilde_pred, x_tilde_train, noise_var, corr_coef, is_training_cov
):
    ...
    for i in range(num_coef):
        for j in range(i + 1):
            if i == j:
                x_tilde_cov[i][j] = tfp.math.psd_kernels.MaternFiveHalves().matrix(
                    x_tilde_pred[i], x_tilde_train[j]
                )
                if is_training_cov:
                    x_tilde_cov_diag = tf.linalg.diag_part(x_tilde_cov[i][j])
                    x_tilde_cov[i][j] = tf.linalg.set_diag(
                        x_tilde_cov[i][j],
                        x_tilde_cov_diag
                        + noise_var[i] * tf.ones_like(x_tilde_cov_diag),
                    )

            else:
                x_tilde_cov[i][j] = tf.sign(
                    corr_coef[i, j]
                ) * tfp.math.psd_kernels.MaternFiveHalves(
                    amplitude=tf.sqrt(tf.abs(corr_coef[i, j]))
                ).matrix(
                    x_tilde_pred[i], x_tilde_train[j]
                )
